...
<script>
import { listByAccount, add, edit, remove } from '@/api/channelRisk'
...
export default {
  ...
  methods: {
    resetTemp() {
      ...
      this.temp.channelId = this.riskChannelId
      this.temp.channelAccountId = this.riskChannelAccountId
    ...
    setUpdateTemp(row) {
      ...
      this.temp.channelAccountId = row.channelAccountId
        ...
        if (valid) {
          ...
          add(this.temp).then(response => {
            if (response.code === 0) {
              ...
              this.handleRisk(this.riskChannelAccountId, this.riskChannelId)
              this.$message({
                type: 'success',
                message: '新增成功!'
              })
